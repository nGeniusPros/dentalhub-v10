 ...
 * - Upload files (PDF, Excel, Word, etc.)
 */
const KnowledgeBaseManager: React.FC = () => {
  ...
  const handleFileUploaded = async (fileData: {
    ...
    try {
      ...
      const { error } = await supabase.from('knowledge_base').insert([
        { 
          content: `File reference: ${fileData.url}`,
          metadata,
          embedding: null // Will be generated by database function
        }
      ]);
      ...
      // Hide file uploader and refresh entries
      ...
      console.error('Error adding file entry:', err);
      setError('Failed to add file to knowledge base');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="p-6">
        ...
        <div className="flex border-b border-gray-200 mb-6">
          ...
          >
            <Icons.Files className="w-4 h-4 inline mr-2" />
            File Entries
          </button>
        </div>
        ...
        {showFileUploader && (
          <div className="bg-gray-50 p-6 rounded-lg mb-6 border border-gray-200">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold">Upload File to Knowledge Base</h2>
              <button
            ...
            <FileUploader onFileUploaded={handleFileUploaded} maxSizeMB={10} />
          </div>
        )}
        ...
        {/* Knowledge Entries Table or File Grid */}
        {activeTab === 'text' ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {isLoading ? (
                  ...
                  ) : entries.filter(e => !e.metadata.fileType || e.metadata.fileType === 'text').length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                    ...
                    entries
                      .filter(e => !e.metadata.fileType || e.metadata.fileType === 'text')
                      .map(entry => (
                        <tr 
                          key={entry.id}
          ...
          <div>
            <h2 className="text-lg font-semibold mb-4">File Entries</h2>
            {isLoading ? (
              <div className="text-center py-10">
                <Icons.Loader className="w-8 h-8 mx-auto text-navy animate-spin" />
                <p className="mt-2 text-gray-500">Loading file entries...</p>
              </div>
            ) : entries.filter(e => e.metadata.fileType && e.metadata.fileType !== 'text').length === 0 ? (
              <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                <Icons.FileQuestion className="w-12 h-12 mx-auto text-gray-400 mb-4" />
                <p className="text-gray-600">No file entries found.</p>
                <p className="text-gray-500 text-sm">Upload files to enhance your knowledge base.</p>
                <Button 
                  onClick={() => setShowFileUploader(true)}
                ...
                {entries
                  ...
                  .map(entry => (
                    <FileEntryCard
                      ...
                      fileType={entry.metadata.fileType || 'File'}
                      fileSize={entry.metadata.fileSize || '0KB'}
                      url={entry.metadata.url || '#'}
                      created={entry.created_at}
                      onDelete={deleteEntry}
                      bundle={entry.metadata.bundle || undefined}
                      bundleName={entry.metadata.bundle ? getBundleName(entry.metadata.bundle) : undefined}
